...
impl Script {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        Ok((
            Script {
                bytes: bytes[consumed..total_len].to_vec(),
            },
            total_len,
        ))
    }
...
impl TransactionInput {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        let (prev_out, consumed1) = OutPoint::from_bytes(bytes)?;
...
impl BitcoinTransaction {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<(Self, usize), BitcoinError> {
        ...
        // lock_time is the last 4 bytes after inputs

        if bytes.len() < 4 {
            return Err(BitcoinError::InsufficientBytes);
        }
        let version = u32::from_le_bytes([bytes[0], bytes[1], bytes[2], bytes[3]]);
        let (count, consumed1) = CompactSize::from_bytes(&bytes[4..])?;
